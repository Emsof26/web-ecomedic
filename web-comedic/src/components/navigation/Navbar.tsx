...
function HomeIcon() {
  return <svg aria-hidden="true" viewBox="0 0 24 24"><path d="m3 11 9-8 9 8v9a1 1 0 0 1-1 1H4a1 1 0 0 1-1-1v-9ZM9 21v-6h6v6" /></svg>;
}

function PatientsIcon() {
  return <svg aria-hidden="true" viewBox="0 0 24 24"><path d="M16 20v-1a4 4 0 0 0-4-4H7a4 4 0 0 0-4 4v1M9.5 11a4 4 0 1 0 0-8 4 4 0 0 0 0 8ZM21 20v-1a4 4 0 0 0-3-3.9M16 3.1a4 4 0 0 1 0 7.8" /></svg>;
}

function ReportIcon() {
  return <svg aria-hidden="true" viewBox="0 0 24 24"><path d="M6 3h8l4 4v14H6a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2ZM14 3v5h5M8 13h8M8 17h6" /></svg>;
}

function ImageIcon() {
  return <svg aria-hidden="true" viewBox="0 0 24 24"><rect x="3" y="4" width="18" height="16" rx="2" /><circle cx="8.5" cy="9" r="1.5" /><path d="m21 16-5-5L5 20" /></svg>;
}
...
function Navbar({ user, onLogout }: NavbarProps) {
  ...
  const isActive = (target: string) => {
    ...
    return location.pathname === "/" && currentHash === target;
  };
  ...
  return (
    <>
      <button className="navbar-toggle" type="button" aria-label={isOpen ? "Cerrar menú" : "Abrir menú"} aria-controls="main-sidebar" aria-expanded={isOpen} onClick={() => setIsOpen((currentValue) => !currentValue)}>
        <MenuIcon />
      </button>
      ...
      <aside id="main-sidebar" className={`navbar${isOpen ? " navbar--open" : ""}`}>
        <div className="navbar__header">
          <a className="navbar__brand" href="/" onClick={closeMenu}>
            <span className="navbar__brand-mark"><img src="/logo/logo-eco.png" alt="Logo EcoMedic" /></span>
            <span><strong>EcoMedic</strong><small>Gestión Ecográfica</small></span>
          </a>
          <button className="navbar__close" type="button" aria-label="Cerrar menú" onClick={closeMenu}><CloseIcon /></button>
        </div>
        ...
        <nav className="navbar__navigation" aria-label="Navegación principal">
          ...
          {!isReceptionist && <NavigationItem href="#nuevo-informe" label="Nuevo Informe Ecográfico" active={isActive("#nuevo-informe")} onNavigate={() => goToSection("#nuevo-informe")}><ReportIcon /></NavigationItem>}
          <NavigationItem href="#repositorio" label="Repositorio de Imágenes" active={isActive("#repositorio")} onNavigate={() => goToSection("#repositorio")}><ImageIcon /></NavigationItem>
          {isAdmin && <NavigationItem href="#configuracion" label="Configuración / Usuarios" active={isActive("#configuracion")} onNavigate={() => goToSection("#configuracion")}><SettingsIcon /></NavigationItem>}
        </nav>

        <div className="navbar__footer">
          <button className="navbar__theme" type="button" aria-pressed={theme === "dark"} onClick={toggleTheme}>
            {theme === "dark" ? <SunIcon /> : <MoonIcon />}
            {theme === "dark" ? "Modo claro" : "Modo nocturno"}
          </button>
          <button className="navbar__logout" type="button" onClick={onLogout}>Cerrar sesión</button>
